---
title: "PredictiveModelingExercises"
date: "8/17/2020"
output:
...
## Report
```{r exercise1, echo=FALSE, results='hide', message=FALSE, fig.show='hide'}
library(mosaic)
library(dplyr)

# Load Data
greenData <- read.csv('Data/greenbuildings.csv')

# Subset into just green buildings (Green Buildings: Rent)
greenBuildings <- greenData %>% filter(green_rating == 1)

# Histogram (Green Buildings: Rent)
hist(greenBuildings$Rent, 25)


# Density (Green Buildings: Rent)
d <- density(greenBuildings$Rent)
plot(d, main="Kernel Density of Rent for Green Buildings")
polygon(d, col="red", border="blue")


# Box Plot (Green Buildings: Rent)
boxplot(greenBuildings$Rent)
...
greenBuildings_Ideal <- greenData %>% filter(green_rating == 1,
                                             ...
                                             amenities==1,
                                             cd_total_07>=966)
# Subset Non-Green Buildings, ~15 stories (+-5 stories), new (built within last 5 years)
...
# Density (Green Buildings (Subset): Rent)
d <- density(greenBuildings_Ideal$Rent)
plot(d, main="Kernel Density of Rent for Green Buildings (Subset)")
polygon(d, col="red", border="blue")

# Density (Non-Green Buildings (Subset): Rent)
d <- density(nonGreenBuildings_Ideal$Rent)
plot(d, main="Kernel Density of Rent for Green Buildings (Subset)")
polygon(d, col="red", border="blue")

# Box Plot (Green Buildings (Subset): Rent)
boxplot(greenBuildings_Ideal$Rent)

# Box Plot (Non-Green Buildings (Subset): Rent)
boxplot(nonGreenBuildings_Ideal$Rent)
...
## Report
```{r exercise2, echo=FALSE, results='hide', message=FALSE, fig.show='hide'}
library(splitstackshape)
library(dplyr)
library(ggmap)

# Load Data
abiaData <- read.csv('Data/ABIA.csv')
airportCodes <- read.csv('Data/airportCodes.csv')
...
```{r exercise2_plot1, echo=FALSE, results='hide', message=FALSE, fig.show='hide'}
...
```
Plots number of depatures from the Austin Airport. Size and color both represent 
number of flight depatures. Dallas and Houston are the two top airports flights 
are flown to out of Austin.
...
```
Plots number of arrival flights to the Austin Airport. Size and color both represent 
number of flight arrivals. Dallas and Houston are the two top airports of incoming
flights to Austin.
```{r exercise2_plot1, echo=FALSE, results='hide', message=FALSE, fig.show='hide'}
...
Maps the average arrival delay for a given airport versus the number of flights 
incoming to Austin. Austin airport officials should be wary of large cirlces that 
are lightly shaded, as these are airports which have expected longer delays 
and have a large number of flights incoming to Austin.
...
Maps the average departure delay for a given airport versus the number of flights 
departing to that airport from Austin.Thankfully, Austin has a good track record 
with their large traffic airport destinations.
...
## Report
```{r exercise3, echo=FALSE, results='hide', message=FALSE, fig.show='hide'}
...
## Report
```{r exercise4, echo=FALSE, results='hide', message=FALSE, fig.show='hide'}
...
## Report
```{r exercise5, echo=FALSE, results='hide', message=FALSE, fig.show='hide'}
...
## Report
```{r exercise6, echo=FALSE, results='hide', message=FALSE, fig.show='hide'}
